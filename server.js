...
app.use((err, req, res, next) => {
  if (err && err.corsRejected) {
    ...
    if (!loggedBlockedOrigins.has(origin)) {
      ...
      console.warn(`[bop] blocking CORS origin: ${origin}`);
    }
    return res.status(403).json({ error: 'origin not allowed' });
  }
  console.error('[bop] request error:', err);
...
async function main() {
  await db.connect();
  console.log(`[bop] MongoDB connected (${config.mongoDb})`);

  getEthPriceUsd().catch(() => {}); // warm the price cache for USD values

  server = app.listen(config.port, () => {
    console.log(`[bop] listening on http://localhost:${config.port}`);
    console.log(`[bop] dryRun=${config.dryRun} chainId=${config.chainId} wallet=${walletAddress()}`);
    if (config.walletIsEphemeral) {
      console.log('[bop] WARNING: using an ephemeral wallet (no WALLET_PRIVATE_KEY set) — dry run only');
    }
    scheduler.start();
  });
}
...
async function shutdown(signal) {
  console.log(`\n[bop] ${signal} received, shutting down`);
...
main().catch((err) => {
  console.error('[bop] failed to start:', err);
  process.exit(1);
});
